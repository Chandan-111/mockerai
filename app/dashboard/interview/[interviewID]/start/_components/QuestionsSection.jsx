...
function QuestionsSection({mockInterviewQuestion,activeQuestionIndex}) {
  ...
  return mockInterviewQuestion&&(
    <div className='p-5 border rounded-lg mt-2'>
        <div className=' grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 flex flex-col gap-5 my-10'>
            {Array.isArray(mockInterviewQuestion) && mockInterviewQuestion.map((question, index) => (
                ...
                className={`p-2 font-bold bg-gray-100 rounded-lg text-center text-xs md:text-sm cursor-pointer
    ...
    <Volume2Icon className='cursor-pointer text-red-500 ml-3 mt-3' onClick={() => textToSpeech(mockInterviewQuestion[activeQuestionIndex]?.question)}></Volume2Icon>
    <h2 className='text-red-500 text-semibold font-medium mt-2'>Click once to hear the question</h2>
    </div>
        <div className='mx-1 my-2 border-2 border-yellow-500 rounded-lg p-2 bg-yellow-50 mt-40'>
